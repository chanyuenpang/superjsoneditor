...
- `field-type-icons-and-accessible-labels.md`: records the shared Tabler icon mapping for seven JSON value types, its object/primitive/reference/array-header render sites, and the tooltip and assistive-technology semantics that must accompany compact type icons.
...
- `adr/use-icons-for-json-type-indicators.md`: records the accepted choice to replace persistent JSON type text with a shared compact icon mapping while retaining tooltip and assistive-technology names.
